...
from utils.RopeSmoothLoss import estimate_pointcloud_local_coord_frames,group_points
import utils.RopeSmoothLoss as RSL
...
class pointsEnhancement(nn.Module):
    def __init__(self,group_nums=40,group_size=70):
        super().__init__()
        self._group_nums = group_nums  # 分组的数量
        self._group_size = group_size  # 分组的大小
    ...
    def forward(self,pointclouds):
        ...
        # 扩展主轴方向的形状以匹配grouped_points
        main_axis_expanded = main_axis.expand_as(grouped_points)  # [b, group_nums, group_size, dim]
        # 将grouped_points和主轴方向的数据沿着最后一个维度连接起来
        enhanced_points = torch.cat([grouped_points, main_axis_expanded], dim=-1)  # [b, group_nums, group_size, dim+dim]
        # 将enhanced_points调整为[b, group_nums*group_size, dim+dim]的形状
...
class RopeSmoothLoss(nn.Module):
    ...
    def forward(self, pointclouds):
        b, N, dim = pointclouds.shape
        _, group_means = RSL.group_points(pointclouds, num_points=self._group_nums, group_size=self._group_size)
        # curvatures [b,group_nums,3] coord_frames [b,group_nums,3,3]
        curvatures, coord_frames,_ = RSL.estimate_pointcloud_local_coord_frames(pointclouds=pointclouds,
                                                                              sampled_points=group_means,
                                                                              neighborhood_size=self._group_size)
        ...
        # third_vec = curvatures[:, :, 0] / (curvatures[:, :, 0] + curvatures[:, :, 1] + curvatures[:, :, 2] + 1e-9)
        loss = -torch.sum(first_vec - second_vec) / b + self._beta * RSL.angular_cost(group_means=group_means,
                                                                                      points_directions=coord_frames[:,
                                                                                                        :, :, 2])
        # print('rope_smooth_loss:',RSL.angular_cost(group_means=group_means,points_directions=coord_frames[:, :, :, 2]))
        return loss
...
class MemoryModule(nn.Module):
    ...
    def forward(self, x,flag=True):
        ...
        if flag:
            self.memory.data = m.max(dim=0, keepdim=True).values.detach()
        return m


@MODELS.register_module()
class MP2CDL(nn.Module):
    ...
    def _get_lossfnc_and_weights(self, config):
        ...
        self.manifold_constraint = ManifoldnessConstraint(support=config.support,
                                                          neighborhood_size=config.neighborhood_size)
        self.rope_smooth_loss = RopeSmoothLoss(group_nums=config.sample_group_num, group_size=config.sample_group_size,
                                               beta=config.rope_direction_weight)
    ...
    def get_loss(self, pts):
        ...
        if self.counter% (154*5) == 0:
            print("Now rope_smooth_weight is: ",self.rope_smooth_weight)
        if self.counter% (154*50) == 0 and self.rope_smooth_weight<0.064:
            self.rope_smooth_weight *=2
